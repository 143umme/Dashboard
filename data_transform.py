...
def plot_cmj_vs_team(df, athlete_name):
    """
    Creates the final CMJ Athlete vs Team Average graph.
    Exactly like your original — now 100% working with your exact fix.
    """
    # === YOUR EXACT LINE — THIS FIXES EVERYTHING ===
    df = df.copy()
    df.columns = df.columns.str.replace('\xa0', ' ', regex=False).str.strip()
    ...
    # === MAIN METRICS ===
    if main_labels:
        athlete_sizes = [abs(v) * 0.5 for v in main_athlete_vals]
        team_sizes = [abs(v) * 0.5 for v in main_team_vals]

        fig.add_trace(go.Scatter(
            x=main_athlete_vals, y=main_labels,
            mode='markers',
            name=athlete_name,
            marker=dict(color='#2ca02c', size=athlete_sizes, sizemode='area', sizemin=20,
        ...
        fig.add_trace(go.Scatter(
            x=main_team_vals, y=main_labels,
            mode='markers',
            name='Team Average',
            marker=dict(color='#ff7f0e', size=team_sizes, sizemode='area', sizemin=20,
                        line=dict(width=1, color='black')),
        ...
        fig.update_xaxes(range=[0, max_main * 1.15], row=1, col=1)
        fig.update_yaxes(categoryorder='array', categoryarray=main_labels[::-1], row=1, col=1)

    # === RSI ===
    if rsi_ok:
        fig.add_trace(go.Scatter(
            x=[rsi_athlete], y=[rsi_label],
            mode='markers', name=athlete_name, showlegend=False,
        ...
        fig.add_trace(go.Scatter(
            x=[rsi_team], y=[rsi_label],
            mode='markers', name='Team Average', showlegend=False,
    ...
    # === INCREASE FONT SIZE & MAKE BLACK + BOLD ===
    fig.update_xaxes(
        tickfont=dict(size=12, family="Arial Black", color="black"),  # X-axis values bigger & bold black
        
        row=1, col=1
    )
    fig.update_xaxes(
        tickfont=dict(size=12, family="Arial Black", color="black"),  # RSI X-axis
        row=2, col=1
    )

    fig.update_yaxes(
        tickfont=dict(size=11, family="Arial Black", color="black"),  # Y-axis metric names bigger & bold black
       
        row=1, col=1
    )
    fig.update_yaxes(
        tickfont=dict(size=11, family="Arial Black", color="black"),  # RSI label
        row=2, col=1
    )
